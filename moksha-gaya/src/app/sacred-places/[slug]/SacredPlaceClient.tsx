...
const particles = [
  { id: 1, size: 4, x: "10%", y: "80%", duration: 20, delay: 0 },
  { id: 2, size: 6, x: "28%", y: "88%", duration: 24, delay: 1 },
  { id: 3, size: 3, x: "46%", y: "76%", duration: 18, delay: 2 },
  { id: 4, size: 5, x: "64%", y: "84%", duration: 26, delay: 3 },
  { id: 5, size: 4, x: "82%", y: "92%", duration: 22, delay: 4 }
];
...
export default function SacredPlaceClient({
  ...
  const handleBooking = (e: React.FormEvent) => {
    ...
    const newLead = saveLead({
      ...
      ritual: `Ritual at ${metadata.place_name || "Gaya Sacred Spot"}`,
      package: "Traditional",
      date: formData.date || new Date().toISOString().split("T")[0],
      additionalInfo: `Requested ritual consultation from sacred place page: ${metadata.place_name || "Unknown"}`
    });
    ...
    const whatsappUrl = getEnquiryWhatsAppUrl(newLead.id, newLead.name, `Ritual at ${metadata.place_name || "Gaya"}`);
      ...
      <section className="relative h-[55vh] min-h-[420px] flex items-end overflow-hidden bg-[#1c1917] text-white">
        ...
        {particles.map((p) => (
          <motion.div
            ...
            animate={{ y: [0, -160], opacity: [0, 0.6, 0] }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              ...
              if (isTestimonial) {
                return (
                  ...
                  >
                    ...
                    <span className="text-xs uppercase font-bold tracking-widest text-[#b17a20]">Devotee Statement</span>
                  </motion.div>
                );
              }
          ...
          <div className="lg:col-span-4 space-y-8 lg:sticky lg:top-24">
            ...
            <div className="bg-white rounded-2xl border border-[#efe9de] p-8 shadow-sm relative overflow-hidden border-t-4 border-t-[#b17a20]">
              <h3 className="font-serif text-xl font-bold text-[#2c1a04] mb-2">Plan Rites Here</h3>
              <p className="text-xs text-[#7c6954] mb-6">
                Consult with our Acharyas to perform Pind Daan or Tarpan at {metadata.place_name}.
              </p>
              ...
              {isBooked ? (
                ...
                >
                  ...
                  <h4 className="font-serif font-bold text-[#2c1a04] text-lg mb-2">Request Submitted</h4>
                  <p className="text-xs text-[#7c6954] leading-relaxed mb-4">
                    Pranam. Your request to plan a ritual at {metadata.place_name} on {formData.date || "preferred date"} has been received.
                    <br/><span className="text-[#b17a20] font-bold">Reference ID: {refId}</span>
                    <br/>Our coordinator will call you back within 2-4 hours.
                  </p>
                  <a
                    href={getEnquiryWhatsAppUrl(refId, formData.name, `Ritual at ${metadata.place_name || "Gaya"}`)}
                    target="_blank"
            ...
            <div className="bg-[#faf8f5] rounded-2xl border border-[#efe9de] p-8">
              ...
              <ul className="space-y-4 text-xs text-[#5c4a37]">
                <li className="flex items-start gap-3">
                  ...
                  <p>Priority coordination at key shrines with local setups.</p>
                </li>
                <li className="flex items-start gap-3">
                  <span className="text-[#b17a20] font-bold">✓</span>
                  <p>Certified, verified local Pandits for precise gotra recitations.</p>
                </li>
                <li className="flex items-start gap-3">
                  <span className="text-[#b17a20] font-bold">✓</span>
                  <p>End-to-end travel, sanitation, and puja logistics support.</p>
                </li>
              </ul>
            </div>

            {/* Other Sacred Places recommendations */}
