      ...
      <body style="margin: 0; padding: 20px; background-color: #faf8f5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
        <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #efe9de; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 12px rgba(44, 26, 4, 0.03);">
          ...
          <div style="background-color: #2c1a04; padding: 30px; text-align: center; border-bottom: 3px solid #b17a20;">
            <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: bold; letter-spacing: 2px; text-transform: uppercase;">Gaya Rituals</h1>
...
export async function POST(req: Request) {
  try {
    ...
    let devoteeSubject = "Thank You for Your Enquiry - Gaya Rituals";
    let devoteeHtmlBody = "";

    if (status === "Confirmed") {
      devoteeSubject = "Booking Confirmed - Gaya Rituals";
      devoteeHtmlBody = `
        ...
        <p style="margin-top: 30px; font-style: italic; color: #b17a20;">Warm regards,<br>The Gaya Rituals Team</p>
      `;
    } else if (status === "Cancelled") {
      devoteeSubject = "Booking Cancelled/Postponed - Gaya Rituals";
      devoteeHtmlBody = `
        <p>Pranam <strong>${name}</strong>,</p>
      ...
      devoteeHtmlBody = `
        <p>Pranam <strong>${name}</strong>,</p>
        <p>Thank you for contacting Gaya Rituals. We have received your enquiry for performing ancestral rites. Your details have been registered under Reference ID: <strong>${leadId}</strong>.</p>
    ...
    if (!isStatusChange) {
      const adminHtmlBody = `
        <p>Pranam Admin,</p>
        <p>A new lead has been registered on the Gaya Rituals portal. Below are the details:</p>
